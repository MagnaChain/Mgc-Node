...
MempoolService.prototype.start = function(callback) {
  ...
  self._db.getPrefix(self.name, function(err, prefix) {
    ...
    log.info('MEMPOOL_SERVICE: mempool disabled until full sync.');
    callback();
  });
};
...
MempoolService.prototype._flushMempool = function(callback) {
  ...
  log.warn('MEMPOOL_SERVICE: flushing mempool, this could take a minute.');
  ...
  var timer = setInterval(function() {
    log.info('MEMPOOL_SERVICE: removed: ' + totalCount + ' records during mempool flush.');
  }, 5000);
  ...
  stream.on('end', function() {
    clearInterval(timer);
    log.info('MEMPOOL_SERVICE: completed flushing: ' + totalCount + ' tx mempool records.');
    callback();
  });
...
MempoolService.prototype.onReorg = function(args, callback) {
  log.info('MEMPOOL_SERVICE: ################ onReorg: ' );
  var removalOps = [];
...
MempoolService.prototype.enable = function() {
  //log.info('MEMPOOL_SERVICE: Mempool enabled.');
  this._startSubscriptions();
  this._enabled = true;
};

MempoolService.prototype.onBlock = function(block, callback) {
  log.info('MEMPOOL_SERVICE: ############ onBlock block.txs.length: ' + block.txs.length);
...
MempoolService.prototype._getAddressOperations = function(tx, reverse) {
  ...
  log.info('MEMPOOL_SERVICE: ################ _getAddressOperations action: ' + action);
  log.info('MEMPOOL_SERVICE: ################ _getAddressOperations tx.outputs.length: ' + tx.outputs.length);
  ...
  log.info('MEMPOOL_SERVICE: ################ _getAddressOperations tx.inputs.length: ' + tx.inputs.length);
...
MempoolService.prototype._onTransaction = function(tx) {
  log.info('MEMPOOL_SERVICE: ################ _onTransaction : ' + JSON.stringify(tx) );
  var self = this;
  var ops = [{
      type: 'put',
      key: self._encoding.encodeMempoolTransactionKey(tx.txid()),
  ...
  log.info('MEMPOOL_SERVICE: ################ _onTransaction ops: ' + JSON.stringify(ops) );
...
MempoolService.prototype.getMempoolTransaction = function(txid, callback) {
  log.info('MEMPOOL_SERVICE: ################ getMempoolTransaction txid : ' + txid );
  var self = this;
...
MempoolService.prototype.getTxidsByAddress = function(address, type, callback) {
  log.info('MEMPOOL_SERVICE: ################ getMempoolTransaction getTxidsByAddress: ' + address );
  var self = this;
  var results = [];
  ...
  stream.on('error', function() {
	log.info('MEMPOOL_SERVICE: ################ getMempoolTransaction getTxidsByAddress error');
    return [];
  });

  stream.on('end', function() {
	log.info('MEMPOOL_SERVICE: ################ getMempoolTransaction getTxidsByAddress end result: ' + JSON.stringify(results) );
    callback(null, results);
  });

  stream.on('data', function(key) {
	log.info('MEMPOOL_SERVICE: ################ getMempoolTransaction getTxidsByAddress data');
