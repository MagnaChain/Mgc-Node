...
var P2P = function(options) {
  //log.info("P2P_SERVICE: P2P  $$$$$$$$$$$$$$$ constructor");
...
P2P.prototype.getP2PBlock = function(opts, callback) {
  log.info("P2P_SERVICE: getP2PBlock $$$$$$$$$$$$$$$$$$$ msg GetBlocks");
  ...
  if (block) {
	log.info("P2P_SERVICE: getP2PBlock get block in cash");
...
P2P.prototype.getHeaders = function(filter) {
  log.info("P2P_SERVICE: getHeaders  ###################### msg GetHeaders");
...
P2P.prototype.getMempool = function() {
  log.info("P2P_SERVICE: getMempool ####################### msg MemPool");
  var peer = this._getPeer();
...
P2P.prototype.sendTransaction = function(tx, callback) {
  log.info("P2P_SERVICE: sendTransaction $$$$$$$$$$$$$$$$$$$$$ msg p2p.Inventory.forTransaction");
  var peer = this._getPeer();
  ...
  log.info('P2P_SERVICE:: sending transaction: ' + bcoinTx.txid());
...
P2P.prototype._disconnectPool = function() {
  ...
  log.info('P2P_SERVICE:: disconnecting pool and peers. SIGINT issued, system shutdown initiated');
...
P2P.prototype.subscribe = function(name, emitter) {
  log.info("P2P.prototype.subscribe");
...
P2P.prototype._applyMempoolFilter = function(message) {
  log.info("P2P.prototype._applyMempoolFilter");
...
P2P.prototype._setRetryInterval = function() {
  log.info("P2P_SERVICE: setRetryInterval");
  var self = this;
...
P2P.prototype._connect = function() {
  ...
  log.info('P2P_SERVICE: Connecting to p2p network.');
  self._pool.connect();
...
P2P.prototype._getBestHeight = function() {
  log.info("P2P_SERVICE: _getBestHeight");
  if (this._peers === 0) {
    return 0;
  }
...
P2P.prototype._initPool = function() {
  //log.info("P2P_SERVICE: _initPool");
  var opts = {};
  if (this._configPeers) {
    opts.addrs = this._configPeers;
  }
  //log.info("################ P2P_SERVICE: _configPeers: " + JSON.stringify(this._configPeers) );
  opts.dnsSeed = false;
  opts.listenAddr = false;
  opts.maxPeers = this._maxPeers;
  opts.network = this.node.network;
  p2p.Pool.RetrySeconds = 3;
  //log.info("P2P_SERVICE: ################ _initPool opts is: " + JSON.stringify(opts) );
...
P2P.prototype._onPeerBlock = function(peer, message) {
  console.log("################### P2P peer on event : block" + JSON.stringify(message.block) );
...
P2P.prototype._onPeerDisconnect = function(peer, addr) {
  //log.info("P2P_SERVICE: _onPeerDisconnect");
  ...
  log.info('P2P_SERVICE: Disconnected from peer: ' + addr.ip.v4);

};

P2P.prototype._onPeerGetData = function(peer, message) {
  // we can only respond to tx messages
  console.log("################### P2P peer on event : getdata" );
...
P2P.prototype._onPeerHeaders = function(peer, message) {
  console.log("################### P2P peer on event : headers" );
...
P2P.prototype._onPeerInventory = function(peer, message) {
  console.log("################### P2P peer on event : inv" );
  var self = this;
  var newDataNeeded = [];
...
P2P.prototype._matchNetwork = function(network) {
  log.info("P2P_SERVICE: _matchNetwork");
...
P2P.prototype._onPeerReady = function(peer, addr) {
  log.info("P2P_SERVICE: _onPeerReady peer: ");
  // clear any interval timers that we previously set
  if (this._retryInterval) {
	//log.info("P2P_SERVICE: ################ _onPeerReady clearInterval");
  ...
  log.info('P2P_SERVICE: Connected to peer: ' + addr.ip.v4 + ', network: ' +
...
P2P.prototype._onPeerTx = function(peer, message) {
  console.log("################### P2P peer on event : tx" );
...
P2P.prototype._startBcoinIfNecessary = function(callback) {
  //log.info("P2P_SERVICE: _startBcoinIfNecessary");
  if (!this._hasPeers()) {
    this._configPeers = [{ ip: { v4: '127.0.0.1'} }];
    //return this._startBcoin(callback);  暂时不开bcoin
  }
  setImmediate(callback);
};
