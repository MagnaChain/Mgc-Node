...
AddressService.prototype.getAddressHistory = function(addresses, options, callback) {
  log.info('ADDRESS_SERVICE: ############### getAddressUnspentOutputs options: ' + JSON.stringify(options) );
  var self = this;
  ...
  async.eachLimit(addresses, 4, function(address, next) {
    log.info('ADDRESS_SERVICE: ############### enter async.eachLimit(addresses, 4, function(address, next)');
...
AddressService.prototype.getAddressUnspentOutputs = function(address, options, callback) {
  ...
  if (_.isUndefined(options.queryMempool)) {
	log.info('ADDRESS_SERVICE: ############### set options.queryMempool true');
    options.queryMempool = true;
  }
  ...
  async.waterfall([
    ...
    function(next) {
      ...
      log.info("ADDRESS_SERVICE: ###############  self._mempool.getTxidsByAddress(address, 'output', next);");
    ...
    function(mempoolTxids, next) {
      ...
      if (mempoolTxids.length <= 0) {
		log.info("ADDRESS_SERVICE: ############### mempoolTxids is empty ");
        return next();
      }
      log.info('ADDRESS_SERVICE: ###############  async.eachLimit(mempoolTxids, 4, function(id, next) ');
    ...
    function(next) {
      log.info('ADDRESS_SERVICE: ############### var utxoStream = self._db.createReadStream(criteria) ');
...
AddressService.prototype._getAddressTxidHistory = function(address, options, callback) {
  log.info('ADDRESS_SERVICE: ############### _getAddressTxidHistory options: ' + JSON.stringify(options) );
  var self = this;
...
AddressService.prototype.onBlock = function(block, callback) {
  log.info('ADDRESS_SERVICE: ############### onBlock');
  var self = this;
...
AddressService.prototype._processInput = function(tx, input, index, opts) {
  //log.info('ADDRESS_SERVICE: ############### _processInput');
  var address = input.getAddress();
...
AddressService.prototype._processOutput = function(tx, output, index, opts) {
  ...
  //log.info('ADDRESS_SERVICE: ############### _processOutput');
  var address = output.getAddress();
...
AddressService.prototype._processTransaction = function(tx, opts) {
  log.info('ADDRESS_SERVICE: ############### _processTransaction');
  var self = this;
