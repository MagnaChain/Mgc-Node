...
var BlockService = function(options) {
  //log.info('BLOCK_SERVICE: BlockService constructor' );
  ...
  //log.info('BLOCK_SERVICE: BlockService options.recentBlockHashesCount is ' + options.recentBlockHashesCount );
  ...
  //log.info('BLOCK_SERVICE: BlockService options.readAheadBlockCount is ' + options.readAheadBlockCount );
  ...
  //log.info('BLOCK_SERVICE: BlockService options.reorgToBlock is ' + options.reorgToBlock );
...
BlockService.prototype.getBlockOverview = function(hash, callback) {
  log.info('BLOCK_SERVICE：getBlockOverview: ');
  var self = this;
...
BlockService.prototype._checkTip = function(callback) {
  ...
  log.info('BLOCK_SERVICE : checking the saved tip...');
  ...
  self._header.getBlockHeader(self._tip.height, function(err, header) {
    ...
    if (header.hash === self._tip.hash && !self._reorgToBlock) {
      log.info('BLOCK_SERVICE: saved tip is good to go.');
      return callback();
    }
...
BlockService.prototype._resetTip = function(callback) {
  log.info('BLOCK_SERVICE _resetTip');
  var self = this;
  ...
  log.warn('BLOCK_SERVICE: resetting tip due to a non-existent tip block...');

  var block;
  var header = self._header.getLastHeader();
  var height = header.height;
  log.info('BLOCK_SERVICE ###############：lastheader.height is ' + header.height );
  self._header.getAllHeaders(function(err, headers) {
    ...
    log.info('BLOCK_SERVICE : retrieved all the headers for lookups.');
	...
	if (!_block) {
          log.debug('BLOCK_SERVICE: block: ' + header.hash + ' was not found, proceeding to older blocks.');
	}
        ...
        if (!block) {
          log.debug('BLOCK_SERVICE: trying block: ' + header.hash);
        }
      ...
      if (err || !block) {
        return callback(err ||
          new Error('BLOCK_SERVICE: none of the blocks from the headers match what is already indexed in the block service.'));
      }
...
BlockService.prototype._performSanityCheck = function(tip, callback) {
  log.info('BLOCK_SERVICE： _performSanityCheck');
  var self = this;
  ...
  async.waterfall([
    ...
    self._performSanityCheck(tip, function(err, tip) {
      ...
      if (!tip) {
		...
		log.info('BLOCK_SERVICE #############： set _tipResetNeeded true');
        return callback();
      }
...
BlockService.prototype._loadRecentBlockHashes = function(callback) {
  log.info('BLOCK_SERVICE： _loadRecentBlockHashes');
  var self = this;
  ...
  async.timesSeries(times, function(n, next) {
    ...
    self.getBlock(hash, function(err, block) {
      ...
      if (err || !block) {
        return callback(err || new Error('BLOCK_SERVICE : attempted to retrieve block: ' + hash +
          ' but was not in the index.'));
      }

      var prevHash = bcoin.util.revHex(block.prevHash);
    ...
    assert(self._recentBlockHashes.length === times, 'BLOCK_SERVICE: did not load enough recent block hashes from the index.');
    log.info('BLOCK_SERVICE: loaded: ' + self._recentBlockHashes.length + ' hashes from the index.');
    callback();
...
BlockService.prototype._getTimeSinceLastBlock = function(callback) {
  log.info('BLOCK_SERVICE：_getTimeSinceLastBlock');
  var self = this;

  self._header.getBlockHeader(Math.max(self._tip.height - 1, 0), function(err, header) {

    if(err || !header) {
      return callback(err || new Error('BLOCK_SERVICE: we should have a header in order to get time since last block.'));
    }
...
BlockService.prototype._queueBlock = function(block) {
  log.info('BLOCK_SERVICE: _queueBlock');
  var self = this;
...
BlockService.prototype._syncPercentage = function() {
  log.info('BLOCK_SERVICE: _syncPercentage');
...
BlockService.prototype._detectReorg  = function(block) {
  ...
  return bcoin.util.revHex(block.prevHash) !== this._tip.hash;
};

BlockService.prototype._getBlock = function(hash, callback) {
  log.info('BLOCK_SERVICE  _getBlock hash: ' + hash);
  var self = this;
...
BlockService.prototype._getHash = function(blockArg, callback) {
  //log.info('@@@@@@@@@@@@@@@@@@@@@ BlockService.prototype._getHash');
...
BlockService.prototype._onReorg = function(commonAncestorHash, block, callback) {
  ...
  async.mapSeries(services, function(service, next) {
    ...
    if(!service.onReorg) {
      log.debug('BLOCK_SERVICE : skipping reorg for: ' + service.name + ' service.');
      return setImmediate(next);
    }

    log.info('BLOCK_SERVICE: Reorging: ' + service.name + ' service.');
...
BlockService.prototype.onHeaders = function(callback) {
 log.info('BLOCK_SERVICE：BlockService onHeaders');
  var self = this;

  if (self._pauseSync) {
    log.warn('BLOCK_SERVICE: pausing sync due to config option.');
    return callback();
  }
...
BlockService.prototype._onHeaders = function(callback) {
  log.info('BLOCK_SERVICE：BlockService _onHeaders');
  ...
  self._resetTip(function(err) {
    ...
    async.retry(function(next) {
	  ...
	  log.info('BLOCK_SERVICE : _onHeaders call _checkTip');
...
BlockService.prototype._startBlockSubscription = function() {
  log.info('BLOCK_SERVICE ############### ： _startBlockSubscription ');
...
BlockService.prototype._findLatestValidBlockHeader = function(callback) {
  log.info('BLOCK_SERVICE：_findLatestValidBlockHeader ');
  var self = this;

  if (self._reorgToBlock) {
    return self._header.getBlockHeader(self._reorgToBlock, function(err, header) {
      if (err || !header) {
        return callback(err || new Error('BLOCK_SERVICE: header not found to reorg to.'));
      }
  ...
  async.until(function() {
    ...
    assert(header, 'BLOCK_SERVICE: we could not locate any of our recent block hashes in the header service ' +
      'index. Perhaps our header service sync\'ed to the wrong chain?');

    assert(header.height <= self._tip.height, 'BLOCK_SERVICE: we found a common ancestor header whose ' +
...
BlockService.prototype._findBlocksToRemove = function(commonHeader, callback) {
  log.info('BLOCK_SERVICE：_findBlocksToRemove ');
  var self = this;
  var hash = self._tip.hash;
  ...
  async.until(function() {
    ...
    self._getBlock(hash, function(err, block) {
      ...
      if (err || !block) {
        return next(err || new Error('BLOCK_SERVICE: block not found in index.'));
      }
      ...
      self._timestamp.getTimestamp(block.rhash(), function(err, timestamp) {
        ...
        hash = bcoin.util.revHex(block.prevHash);
        height--;
...
BlockService.prototype._handleReorg = function(callback) {
  log.info('BLOCK_SERVICE: _handleReorg ');
  var self = this;
    ...
    function(next) {
      ...
      self._findBlocksToRemove(commonAncestorHeader, function(err, _blocksToRemove) {
        ...
        assert(blocksToRemove.length >= 1 && blocksToRemove.length <= self._recentBlockHashes.length,
          'BLOCK_SERVICE: the number of blocks to remove looks to be incorrect.');

        log.warn('BLOCK_SERVICE: chain reorganization detected, current height/hash: ' + self._tip.height + '/' +
...
BlockService.prototype._processReorg = function(commonAncestorHeader, blocksToRemove, callback) {
  log.info('BLOCK_SERVICE: _processReorg ');
  var self = this;
  var operations = [];
  var blockCount = 0;
  var bar = new utils.IndeterminateProgressBar();
  ...
  async.eachSeries(blocksToRemove, function(block, next) {
    ...
    log.info('BLOCK_SERVICE : removed ' + blockCount + ' block(s) during the reorganization event.');
...
BlockService.prototype._onBlock = function(block, callback) {
  log.info('BLOCK_SERVICE：_onBlock ');
  var self = this;
  ...
  self._getBlock(block.rhash(), function(err, _block) {
    ...
    if (_block) {
      self._processingBlock = false;
      log.debug('BLOCK_SERVICE : not syncing, block already in database.');
      return callback();
    }
...
BlockService.prototype._processBlock = function(block, callback) {
  log.info('BLOCK_SERVICE：_processBlock ');
  var self = this;
  ...
  log.info('BLOCK_SERVICE： new block: ' + block.rhash());
...
BlockService.prototype._saveBlock = function(block, callback) {
  log.info('BLOCK_SERVICE：_saveBlock ');
  var self = this;
  ...
  async.mapSeries(services, function(service, next) {
    ...
    log.info('BLOCK_SERVICE: ################## service call ' + service.name + ' onBlock');
    ...
    log.info('BLOCK_SERVICE: ################## ops is  ' + JSON.stringify(ops) );
    self._db.batch(_.compact(_.flattenDeep(ops)), function(err) {
      ...
      self._recentBlockHashes.set(block.rhash(), bcoin.util.revHex(block.prevHash));
...
BlockService.prototype._syncBlock = function(block) {
  ...
  self._saveBlock(block, function(err) {
    ...
    if (self._tip.height < self._header.getLastHeader().height) {
	  log.info('BLOCK_SERVICE ##############：_syncBlock emit next block');
...
BlockService.prototype.onBlock = function(block, callback) {
  log.info('BLOCK_SERVICE ###########：onBlock ');
  var self = this;
...
BlockService.prototype._setTip = function(tip, callback) {
  log.info('BLOCK_SERVICE: Setting tip to height: ' + tip.height + ' ,hash: ' + tip.hash );
  //log.debug('BLOCK_SERVICE: Setting tip to hash: ' + tip.hash);
  this._tip = tip;
...
BlockService.prototype._logSynced = function() {
  ...
  self._getTimeSinceLastBlock(function(err, diff) {
    ...
    log.info('BLOCK_SERVICE: The best block hash is: ' + self._tip.hash +
...
BlockService.prototype._onSynced = function() {
  log.info('BLOCK_SERVICE：_onSynced ');
  var self = this;
...
BlockService.prototype._startSync = function() {
  log.info('BLOCK_SERVICE：_startSync ');
  var numNeeded = Math.max(this._header.getLastHeader().height - this._tip.height, 0);

  log.info('BLOCK_SERVICE : Gathering: ' + numNeeded + ' block(s) from the peer-to-peer network.');

  if (numNeeded > 0) {
    this.on('next block', this._sync.bind(this));
    this.on('synced', this._onSynced.bind(this));
    clearInterval(this._reportInterval);
	log.info('BLOCK_SERVICE：BlockService setInterval _logProgress ');
...
BlockService.prototype._sync = function() {
  ...
  log.debug('BLOCK_SERVICE：querying header service for next block using tip: ' + self._tip.hash);
  ...
  self._header.getEndHash(self._tip, self._readAheadBlockCount, function(err, targetHash, endHash) {
    ...
    self._getBlocksTimer = setTimeout(function() {
      log.debug('BLOCK_SERVICE：block timeout, emitting for next block');
      self._processingBlock = false;
      if (!self._reorging) {
		log.info('BLOCK_SERVICE：_sync emit next block');
    ...
    log.info("BLOCK_SERVICE : ################# _sync call p2p.getP2PBlock ");
...
BlockService.prototype._logProgress = function() {
  ...
  log.info('BLOCK_SERVICE：download progress: ' + this._tip.height + '/' +
    bestHeight + '  (' + progress + '%)');
